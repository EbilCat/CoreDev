...
{
    [CustomEditor(typeof(HeightRecalculatedLayoutElement), true)]
    [CanEditMultipleObjects]
    public class HeightRecalculatedLayoutElementEditor : Editor
    {
        SerializedProperty heightSource;
        ...
        {
            heightSource = serializedObject.FindProperty("heightSource");
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!m_IgnoreLayout.boolValue)
            {
                ...
                EditorGUILayout.PropertyField(heightSource);
                EditorGUILayout.Space();

                LayoutElementField(m_MinWidth, 0);
                LayoutElementField(m_MinHeight, 0);
                HeightRecalculatedLayoutElementField(m_PreferredWidth, t => t.rect.width);
                HeightRecalculatedLayoutElementField(m_PreferredHeight, t => t.rect.height);
        ...
        {
            HeightRecalculatedLayoutElementField(property, _ => defaultValue);
        }

        void HeightRecalculatedLayoutElementField(SerializedProperty property, System.Func<RectTransform, float> defaultValue)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                property.floatValue = (enabled ? defaultValue((target as HeightRecalculatedLayoutElement).transform as RectTransform) : -1);
            }
